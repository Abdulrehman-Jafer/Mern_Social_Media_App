...
const Sidebar = ({ fixed }: { fixed?: boolean }) => {
  ...
  const showSearch = () => setSearchDisplay(true)

  const hideSearch = () => setSearchDisplay(false)

  const showCreatePost = () => setCreatePostDisplay(true)

  const hideCreatePost = () => setCreatePostDisplay(false)
  ...
  return (
    <>
      <main className={`flex flex-col gap-10 p-4 text-white border-r-2 w-[20%] border-gray-500 min-h-[100vh] ${fixed ? "fixed" : ""} mediaHidden`}>
        <h2 className='text-3xl specificFont bg-transparent backdrop-blur-lg text-center cursor-default'>Spieser</h2>
